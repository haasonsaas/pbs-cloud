...
- H2 `/finish` drains the request body and returns `data: null` (matches upstream behavior).
...
- Helm chart defaults GC interval to 6 hours (tune `PBS_GC_INTERVAL_HOURS` as needed).
